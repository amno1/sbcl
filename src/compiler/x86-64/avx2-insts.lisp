...
(define-instruction-format (evex 32)
  ...
  (evex-u   :field (byte 1 10))
  (reserved :field (byte 1 11) :value 0)                ; bit 3 remains reserved (must be 0)
  (mm       :field (byte 2 8))                          ; bits 1:0 of byte 1
  ;; Byte 2
...
(define-instruction-format (evex-op (+ 32 8)
                                    :include evex
                                    :default-printer '(:name))
    (op :field (byte 8 32)))
...
;;; VEX Mask Register Formats (Needed for KTEST, KAND, KOR, etc.)
;; (define-instruction-format (vex2 16)
;;   (vex :field (byte 8 0) :value #xC5)
;;   (r :field (byte 1 (+ 8 7)) :type 'vex-r)
;;   (vvvv :field (byte 4 (+ 8 3)) :type 'ymm-vvvv-reg)
;;   (l  :field (byte 1 (+ 8 2)) :type 'vex-l)
;;   (pp :field (byte 2 (+ 8 0)))
;;   ;; Implicit bits for 2-byte VEX. This allows the disassembler
;;   ;; to correctly reject instructions requiring W=1 (like KTESTQ)
;;   (m-mmmm :field (byte 0 0) :value 1)
;;   (w :field (byte 0 0) :value 0))
...
(define-instruction-format (vex3-kreg-reg/mem-imm 24
                            :default-printer '(:name :tab reg ", " reg/mem ", " imm8))
  (pp     :field (byte 2 0))
  (m-mmmm :field (byte 5 0))
  (op     :field (byte 8 0))
  (w      :field (byte 1 7))
  (l      :field (byte 1 2)))
...
(eval-when (#-sb-xc :compile-toplevel :load-toplevel :execute)
  ...
  ;; Return (values mm u) for the given opcode map number.
  ;; Map 1 = 0F, 2 = 0F38, 3 = 0F3A, 5 = MAP5, 6 = MAP6, etc.
  (defun evex-encode-map (opcode-prefix)
    (ecase opcode-prefix
      ((1 #x0F) (values #b01 0))   ; 0F
      ((2 #x0F38) (values #b10 0)) ; 0F38
      ((3 #x0F3A) (values #b11 0)) ; 0F3A
      ((9 5) (values #b01 1))      ; MAP5
      ((10 6) (values #b10 1))))   ; MAP6
  ...
  ;;     (2 (values #b10 0))
  ;;     (3 (values #b11 0))
  ;;     ;; Add correct mappings for MAP5/6. These are examples and may need verification.
  ;;     (5 (values #b01 1))               ; MAP5 typically uses mm=01, U=1
  ;;     (6 (values #b10 1))               ; MAP6 typically uses mm=10, U=1
  ;;     (t (values #b00 0))))
  )
...
(defun emit-evex (segment r x b r-prime map w vvvv pp z ll evex-b v-prime aaa)
  (multiple-value-bind (mm u) (evex-encode-map map)
    (emit-bytes segment
                #x62
                ;; P0: R X B R' U 0 mm
                (logior (ash (logxor 1 r) 7)
...
(defun emit-avx512-inst (segment thing reg prefix opcode
                         &key (remaining-bytes 0)
                              ll
                              (map 1)      ; map number: 1=0F, 2=0F38, 3=0F3A, 9=MAP5, 10=MAP6
                              (w 0)
                              vvvv
                              (aaa 0)
                              (z 0)
  ...
  (multiple-value-bind (ll r x b r-prime v-prime)
    ...
    (let ((vvvv-num (if vvvv (reg-id-num (reg-id vvvv)) 0)))
      (emit-evex segment r x b r-prime map w
...
(defun emit-avx2-inst (segment thing reg prefix opcode
  ...
  ;;
  ;; This must stay enabled. VEX has only 4 bits for a register, so an
  ;; operand in XMM16-31 silently truncates to its low 4 bits - a wrong
  ;; register, no error. Instructions on *ALREADY-HAND-DEFINED* (VMOVQ
  ;; among them) reach the assembler through here rather than through
  ;; the generated avx512.insts.lisp, so they depend on this promotion
  ;; to address the high register file at all. With it disabled,
  ;; SIMD-PACK-512.LISP's %MAKE-SIMD-PACK-512-UB64 wrote its lanes to
  ;; XMM15/14/13 (truncated from the XMM31/30/29 the register
  ;; allocator actually assigned) while the EVEX VINSERTI64X2 that
  ;; combines them read the untouched XMM31/30/29, so every lane above
  ;; the first two came back zero.
  (flet ((evex-reg-p (r)
           (and (register-p r)
    ...
    (when (or (evex-reg-p reg)
      ...
      (return-from emit-avx2-inst
        (emit-avx512-inst segment thing reg prefix opcode
                          ...
                          :ll (if (eql l 0) 0 nil)
                          ;; EMIT-AVX512-INST takes a map NUMBER, while the
                          ;; VEX side of this function speaks opcode-prefix
                          ;; BYTES. That rename is what stranded this
                          ;; promotion as dead commented-out code.
                          :map (ecase opcode-prefix
                                 (#x0F 1) (#x0F38 2) (#x0F3A 3))
                          :w (or evex-w w)
                          :vvvv vvvv
                          :vm vm
...
(eval-when (#-sb-xc :compile-toplevel :load-toplevel :execute)
  ...
  (defun avx512-inst-printer-list (inst-format-stem prefix opcode
                                 &key more-fields printer
                                      ...
                                      nds
                                      disp-n)
  (multiple-value-bind (mm u) (evex-encode-map opcode-prefix)
  ...
  (defun avx2-inst-printer-list (inst-format-stem prefix opcode
    ...
    (let ((fields `((pp ,(vex-encode-pp prefix))
      ...
      (append
       ...
       ;; except when evex is explicitly t (e.g. vpinsr, vpextr).
       (when (or evex
                 (= opcode-prefix #x0F)
